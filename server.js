...
setInterval(() => {
  http.get(`http://macbott.glitch.me/`);
}, 280000);
...
client.on('ready', function(){
    var ms = 10000 ;
      var setGame = ['#help | قائمة الأوامر','  created by ryad and anis ',`${client.guilds.size} servers`,`i help ${client.users.size} Members`,'#inv | لأضافة البوت',''];
    var i = -1;
    var j = 0;
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`http://www.twitch.tv/FM`);
    }, ms);
...
client.on('message',anis => {
  
if (anis.author.bot) return;
if (anis.content === prefix+'help') {
  
anis.channel.sendMessage("  ⦁ =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-⦁\n<a:5701:611543044243849216>  | ** `#public` قائمة الأوامر العامة **  \n<a:3863:611545401878904863>  | ** `#admin` قائمة الأوامر الإدارية **  \n<a:ezgif:611546771285344265>  | ** `#music`قائمة اوامر الميوزك **    \n<a:ngameboy:611548032558694410>  | **`#games` قائمة اوامر العاب ** \n=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.=.\n ANIS AND Ryad  البوت خاص ب  \n ⦁ =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-⦁ ") 
}
})
...
#bk **رسالة لجميع اعضاء السيرفر **
${prefix}** move @user | لسب الشخص الى روومك **
${prefix}** rolebc <everyone or @role> | ارسال رساله جماعيه لرتبه محددة **
...
fs.writeFile("./creditsCode.json", JSON.stringify(credits), (err) => {
  ...
  credits[message.author.id] = {
    ...
    if(message.content.startsWith(prefix + "gggggggggg" || prefix + "gggggg")) {
...
client.on('message',async message => {
    ...
    if(args[0].toLowerCase() === `help`) {
            ...
            .addField(``)
            await message.channel.send(`<a:5555:603613426358681600> , **هذه قائمة بجميع اوامر البووت.**`);
    ...
    } else if(args[0].toLowerCase() === `#mtickets`) {
		...
		if(args[1] && args[1].toLowerCase() === "enable") {
			mtickets = true;
			message.channel.send(` , **تم تفعيل التكتات , الاَن يمكن لأعضاء السيرفر استخدام امر انشاء التكت**`);
		} else if(args[1] && args[1].toLowerCase() === "disable") {
			mtickets = false;
			message.channel.send(` , **تم اغلاق نظام التكتات , الاَن لا يمكن لأي عضو استخدام هذا الأمر**`);
		} else if(!args[1]) {
			if(mtickets === true) {
			mtickets = false;
			message.channel.send(` , **تم اغلاق نظام التكتات , الاَن لا يمكن لأي عضو استخدام هذا الأمر**`);
		...
		message.channel.send(`, **سيتم اغلاق الروم في 3 ثواني من الاَن.**`);
		...
		message.channel.send(`, **جارى اعادة تشغيل البوت.**`);
		client.destroy();
		...
		for(let q = 0; q < tchannels.length; q++) {
			...
			if(q === tchannels.length - 1 || q === tchannels.lengh + 1) {
				message.channel.send(` , **تم مسح \`${iq}\` من التكتات.**`);
			}
		}
	}
});
client.on('message', message => {
  if (message.channel.id === "610809081036406803") {
    message.react('☑')
      .then(() => {
        message.react('🚫')
      });
...
if (!message.channel.guild) return;
if (message.author.bot) return;
let sug = message.guild.channels.find("name", "كل-الإقتراحات");
...
message.guild.channels.find('name', 'كل-الإقتراحات').send(embed);
...
client.on('ready',async () => {
  let GUILDID = '608081807828582412'; // اي دي السيرفر
  let CHANNELID = '610808709022482453'; // اي دي الروم
...
client.on('message', async message => {
    let amount = 250;
    if(message.content.startsWith(prefix + "للللللللللل")) {
    if(message.author.bot) return;
...
client.on('message', message => {
                if(message.content === prefix + "inv") {
                    ...
                    let embed = new Discord.RichEmbed ()
                    embed.setTitle(" Click TO AD BOT | ** Invite MAC BOT ™!**")
                    .setURL("https://discordapp.com/api/oauth2/authorize?client_id=610557673322053633&permissions=8&scope=bot");
...
client.login("NjEzMDk0OTk4Njk4ODg1MTQx.XVr7QA.AYeC5Uz_Mmgpvns6oxZe_-4q0UA");
